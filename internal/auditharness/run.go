...
func runFixture(ctx context.Context, rule string, fixture Fixture, opts Options) FixtureResult {
	...
	testPass, output, err := runGNOTest(ctx, opts.GNOBin, fixture.Path)
...
func runGNOTest(ctx context.Context, gnoBin, dir string) (bool, string, error) {
	if gnoBin == "" {
		gnoBin = "gno"
	}
