...
func RegisterProfileAdd(s *server.Server, gnowebClient *http.Client, verify ChainIDVerifier, onAdded func() error) {
	s.Registry().Add(&server.Tool{
		...
		Description: "Adds a chain profile to this gnomcp process, in-memory only — it lasts until restart " +
			...
			"dev and known testnets (test*, topaz-*, sapphire-*) are write-capable; any other chain (mainnet/betanet, e.g. gnoland1) is " +
...
func profileAddInputSchema() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			"name": map[string]any{
				"type": "string",
				"description": "Profile name: lowercase alphanumeric, internal '-' or '_' allowed (e.g. 'sapphire'). " +
			...
			"rpc_url": map[string]any{
				"type":   "string",
				"format": "uri",
				"description": "Chain RPC endpoint (absolute http(s) URL, e.g. 'https://rpc.sapphire.testnets.gno.land'). " +
					"Required together with chain_id unless gnoweb_url is given.",
			},
			"chain_id": map[string]any{
				"type": "string",
				"description": "Chain-id the node reports (e.g. 'sapphire-1', or 'gnoland1' for betanet — read-only). " +
					"dev and known testnets (test*, topaz-*, sapphire-*) are write-capable; any other id is admitted read-only. " +
					"Required together with rpc_url unless gnoweb_url is given. Cross-checked against the live node.",
			},
			"gnoweb_url": map[string]any{
				"type":   "string",
				"format": "uri",
				"description": "Gnoweb page URL to discover rpc_url + chain_id from (e.g. 'https://sapphire.testnets.gno.land'). " +
			...
			"tx_indexer_url": map[string]any{
				"type":   "string",
				"format": "uri",
				"description": "Optional tx-indexer GraphQL endpoint (e.g. 'https://indexer.sapphire.testnets.gno.land/graphql/query'); " +
