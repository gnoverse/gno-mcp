...
func RegisterConnect(s *server.Server, client *http.Client) {
	s.Registry().Add(&server.Tool{
		...
		Description: "Discovers how to connect to a gno chain from its gnoweb URL: reads the " +
			...
			"'gnomcp profile add' command for the user to persist it. dev and known testnets (test*, topaz-*, sapphire-*) are " +
			...
			"Required: gnoweb_url (e.g. 'https://gno.land/r/gnoland/blog' or 'https://sapphire.testnets.gno.land'). " +
		...
		InputSchema: map[string]any{
			...
			"properties": map[string]any{
				"gnoweb_url": map[string]any{
					"type":        "string",
					"format":      "uri",
					"description": "Base gnoweb URL of the chain (e.g. 'https://sapphire.testnets.gno.land').",
				},
				"name": map[string]any{
					"type":        "string",
					"description": "Suggested profile name (e.g. 'sapphire'). Default: the chain-id.",
				},
			},
