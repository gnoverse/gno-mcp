...
func TestServer_fund(t *testing.T) {
	...
	resp, err := http.Post(srv.URL+"/fund", "application/json", strings.NewReader(`{"address":"g1abc","chain_id":"test5"}`))
	...
	resp2, err := http.Post(srv.URL+"/fund", "application/json", strings.NewReader(`{"address":"g1abc","chain_id":"test5"}`))
	require.NoError(t, err)
