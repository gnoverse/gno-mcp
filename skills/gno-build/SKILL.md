...
1. **Load the knowledge.** Read `../gno/SKILL.md` (the source index), then `../gno/references/build.md` (project layout, `gnomod.toml`, the `gno` binary, test flavors) and `../gno/references/patterns.md` (realm idioms — globals, `init`, crossing-function discipline, state shape). Pull `interrealm.md` for `cur realm` / crossing / caller-identity semantics, `stdlib.md` for the chain API, `render.md` if it exposes `Render()`, `security.md` to avoid known footguns. Load what the realm needs, not all of them. If the realm interacts with a system realm (registers a name via `r/sys/users`, reads chain params, touches the validator set), pull `../gno/references/gnoland.md` — and query the target chain for the sys realm's live API rather than coding against remembered signatures.
