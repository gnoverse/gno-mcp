...
by the chain's CLA gate is cleared with `gno_cla_sign` — follow that tool's two-step flow (fetch,
show the agreement URL to the user, confirm, sign) rather than hand-rolling a `gno_call` to
`r/sys/cla`.
