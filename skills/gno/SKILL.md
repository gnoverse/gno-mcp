...
| Platform / sys realms | `references/sysrealms.md` | Reasoning about gno.land system realms (`r/sys/*`: names/users/namereg, validators, params, cla), namespace/name registration, the validator set, genesis-predeployed realms, GovDAO param governance, or why a sys realm differs across networks (sapphire vs local vs betanet). Teaches the design; sends you to the live chain for concrete values. |
| Live testnets / cross-chain | `references/networks.md` | Targeting a specific live testnet or porting between them — per-chain facts (gas price, CLA gate state, faucet, toolchain tag) for sapphire (current) and topaz (sunset), plus what did and did not drift between them in shared imports. Load before writing code destined for a named chain or moving code between chains. |
| Failure triage | `references/debug.md` | Triaging a call or tx that already failed — the failure-signature → root-cause → fix table (structured gnomcp error codes and raw chain strings), the reproduce-cheaply-before-rebroadcast order, the funding-class exception, and the indexer tools for a postmortem. The companion to gnokey.md: that one explains the cost model, this one reads the error. |
...
- **Quarantine snapshot (frozen at the test-13 cut).** The kept-in-`examples/` set was frozen at scaffold time; packages added/removed since are not reflected, and the live testnet may not carry a listed package at all — e.g. `p/demo/tokens/grc721` is in the tables but is deployed on neither sapphire nor topaz. Verify each import against the target chain, not just master.
