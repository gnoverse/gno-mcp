...
- *Answering "how do I register a name / who are the validators / is enforcement on?" for a specific chain* → `gnoland.md` (the sys-realm model + the trust-the-chain query discipline) + `mcp.md` (the query tools). The answer is a live query, not a recital.
