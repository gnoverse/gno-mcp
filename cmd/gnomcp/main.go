...
const serverInstructions = "gnomcp exposes Gno chain operations over MCP. The indexer and faucet tools register only when a profile provides them (tx-indexer-url / a testnet); the rest are always available. Typical flows:\n" +
	...
	"- WRITE on testnet: gno_key_generate (once) -> gno_faucet_fund (fund the agent key) -> gno_call / gno_run / gno_addpkg. An unfunded write returns insufficient_funds pointing at gno_faucet_fund. A deploy rejected with cla_unsigned needs the chain's CLA signed once per key: gno_cla_sign fetches the agreement URL and hash — show the URL to the user and get their confirmation, then call it again with confirmed=true.\n" +
