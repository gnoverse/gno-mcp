...
const serverInstructions = "gnomcp exposes Gno chain operations over MCP. The indexer and faucet tools register only when a profile provides them (tx-indexer-url / a testnet); the rest are always available. Typical flows:\n" +
	...
	"- DISCOVER: when the user names an app or realm without its full path, enumerate the chain — gno_packages (path prefix or @namespace), or gno_list where an indexer is configured — instead of guessing package paths or asking the user for something the chain can answer. When the user names a CHAIN or network (\"on sapphire\", \"on topaz\"), gno_profile_list maps profile names to chain-ids and endpoints — the startup list below can be stale after runtime adds.\n" +
