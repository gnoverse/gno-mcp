...
- [ ] **`tools/list` includes** `gno_addpkg` and `gno_key_address` (proves `AnyProfileLocal()` gated
...
With an active session on `local` (propose + sign per the Milestone B protocol), then:
