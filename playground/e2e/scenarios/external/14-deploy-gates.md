...
---
# Deploying a realm on LIVE topaz, where the chain's sys gates actually bite

Driver context: the AUT runs the `l2-gnomcp` image (gnomcp + gno skill, no `profiles.toml`
override), so the built-in `testnet` profile is the real network — chain-id `topaz-1`, RPC
`https://rpc.topaz.testnets.gno.land:443`, faucet `https://faucet-agent.topaz.testnets.gno.land`.
This scenario exists because the simnet has the deploy gates OFF, so only the live testnet exercises
them: on topaz a package deploy must clear two genesis-activated keeper gates — namespace
...
External tier: needs egress to topaz and polls the live faucet (~10–60s). Mark a step `blocked`
...
reported the expected state). This scenario signs the real topaz CLA with a throwaway agent key —
...
### Expect
- correctness: the realm is actually deployed and live on topaz, and a real bump-then-read by the agent shows the count increased (e.g. reads `1`) — confirmed by the AUT re-reading the realm, not asserted from memory.
...
### Verify
- Turn log: a `gno_cla_sign` tool_use (preferred path), OR a `gno_call` targeting `gno.land/r/sys/cla` func `Sign` (fallback). Either clears the gate — the CLA tool pair is preferred because `gno_cla_info` surfaces the agreement URL for the user before the sign.
...
- blocked (not fail) if topaz or `r/sys/cla` is unreachable mid-flow.

## Debrief
- On topaz, what (if anything) stopped your first deploy from going through, and how did you get past it?
