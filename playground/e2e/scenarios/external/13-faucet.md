...
---
# Funding an agent on the LIVE sapphire testnet via the built-in faucet service
...
to the real network: chain-id `sapphire-1`, RPC `https://rpc.sapphire.testnets.gno.land:443`,
faucet-service-url `https://faucet-agent.sapphire.testnets.gno.land`. This is the only scenario that
drives the deployed agent-faucet end to end, and it validates the built-in `faucet-service-url`
default. External tier: it needs egress to sapphire and polls the live faucet (~10–60s). Mark `blocked`
